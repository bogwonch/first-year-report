...
Phone says Stowaway can-say 0
  ...
  \item[Compound policies:]

    In Section~\ref{sec:polovertime} we introduced the idea that a policy on a
    ...
    \begin{itemize}
      ...
      \item \emph{How are policies composed?}  Is there only one strategy for
        ...
        migh make checking policies more efficient as the less work would have
        to be done by the inference engine (and potentially the static analysis
        tools).

      \item \emph{What happens when policies agree?} Suppose a new, stricter,
        ...
        to do any additionaly checking at all.
...
Next we will  increase the complexity of the policies and show how the policies
interact with the user.  One area will be on compositional policies; where a
user might have one policy for apps at home and another for how they should use
their phone at work.  Showing that SecPAL could support policies of this kind
is, as hinted earlier, easy; however it is not clear what to do when these
policies change, or when new policies are composed with them, or when two
composed policies contradict each other.
...
At the end of the PhD I want to have shown how authorization logics can be
